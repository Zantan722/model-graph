...
const labels:Record<string,string>={interrupted:'已中斷',running:'分析中',reading:'追查來源',synthesizing:'整理說明',pending:'等候分析',described:'已整理說明',proposed:'尚未產圖',queued:'本批排隊中',generating:'產圖中',ready:'已完成',complete:'完成',partial:'部分完成',failed:'未完成',cancelled:'已取消',awaiting_selection:'等待選圖'};
...
export function ProjectReportView({report,onPreview,onRefine,onGenerate,canGenerate,disabled}:{report:ProjectReport;onPreview:(source:string)=>void;onRefine:(view:ProjectView)=>void;onGenerate:(ids:string[])=>void;canGenerate:boolean;disabled?:boolean}){
 ...
 return <section className="project-report" aria-label="Repo 分析結果">
 <header><span className="eyebrow">REPOSITORY ATLAS</span><h2>{report.title}</h2><p>{report.summary}</p><div className="report-counts"><strong>{report.themes.length} 主題</strong><strong>{all.length} 份視圖說明</strong><strong>{ready} 張已完成</strong><span>{labels[report.status]||report.status}</span>{report.diagram&&<span>{diagramTypes[report.diagram].name}{report.diagram==='c4'?` / ${report.level}`:''}</span>}</div></header>
 <p>{report.project} · 已讀 {report.coverage.read.length}/{report.coverage.indexed} 份可讀檔案。來源已核對，分析語意仍需確認。</p>
 {report.error&&<p role="alert" className="puml-error">{report.error}</p>}
 <div className="view-selection-bar"><div><strong>先讀說明，再選擇要畫的圖</strong><p>未勾選不產圖。可分批選擇；已完成的圖不會重跑。</p></div><div className="ai-actions"><button disabled={locked||!eligible.length} onClick={()=>setSelected(ids=>[...new Set([...ids,...eligible.filter(v=>visible.some(t=>t.views.some(item=>item.id===v.id))).map(v=>v.id)])])}>選取篩選結果</button><button disabled={disabled||!chosen.length} onClick={()=>setSelected([])}>清除選取</button><button className="primary" disabled={locked||!canGenerate||!chosen.length} onClick={()=>{onGenerate(chosen);setSelected([]);}}>產生所選 {chosen.length} 張圖</button></div><p role="status">已選 {chosen.length} 張／可選 {eligible.length} 張</p>{!report.snapshotAvailable&&<p>{disabled?'正在分析並保存來源，完成後即可選圖。':'此報告沒有可用的來源快照，請重新分析；既有圖仍可預覽。'}</p>}{!canGenerate&&!disabled&&<p>請在左側選擇 Codex CLI 或 Claude Code，以產生所選圖表。</p>}</div>
 <div className="ai-actions"><button onClick={()=>download('model-graph-project-report.json',JSON.stringify(report,null,2),'application/json')}>下載完整報告與 PUML</button></div>
 <label>篩選主題或視圖<input value={filter} onChange={e=>setFilter(e.target.value)} placeholder="例如 身分驗證、訂單或部署"/></label>
 {visible.map(theme=><section className="theme-section" key={theme.id}><header><span className="small-label">{labels[theme.status]||theme.status}</span><h3>{theme.title}</h3><p>{theme.question}</p><p>{theme.rationale}</p></header>{theme.error&&<p className="puml-error">{theme.error}</p>}{theme.gaps.length>0&&<details><summary>分析缺口 · {theme.gaps.length}</summary>{theme.gaps.map((gap,i)=><p key={i}>{gap}</p>)}</details>}
 ...
 <details><summary>{itemLabels[view.content.kind]} · {view.content.items.length} 項與來源</summary><ul>{view.content.items.map((item,i)=><li key={i}><strong>{item.title}</strong><p>{item.description}</p>{item.evidence.map((e,j)=><details key={j}><summary>{e.path}:{e.startLine}–{e.endLine}</summary><pre>{e.excerpt}</pre></details>)}</li>)}</ul></details>
 <p>待確認：{view.uncertainty}</p><div className="ai-actions">{view.source&&<><button className="primary" onClick={()=>onPreview(view.source!)}>預覽／載入視圖</button><button onClick={()=>download(`${view.id}.puml`,view.source!,'text/plain')}>下載 PUML</button><button disabled={disabled} onClick={()=>onRefine(view)}>帶回 Prompt 修改</button></>}{view.draftSource&&<button onClick={()=>onPreview(view.draftSource!)}>修正未通過的草稿</button>}</div>{view.error&&<p className="puml-error">{view.error}</p>}{view.warnings?.map((w,i)=><p key={i}>圖表提醒：{w.message}</p>)}
 </article>)}</section>)}
 {!all.length&&!disabled&&<p>目前沒有足夠來源可建議視圖，請查看分析缺口並調整範圍。</p>}
 <details><summary>來源覆蓋與內容指紋</summary><p>所選圖表使用分析時的本機來源快照；要納入程式碼的新變更，請重新分析。未宣稱涵蓋所有執行時行為。</p>{report.coverage.read.map(f=><p key={f.path}>{f.path}<code>{f.sha256.slice(0,16)}</code></p>)}{report.coverage.omitted.map(f=><p key={f.path}>略過 {f.path}：{f.reason}</p>)}</details>
 <p>視圖說明、來源快照與已完成圖表保存在本機，下次啟動可繼續選圖。畫布編輯可另存至流程庫。</p>
 </section>;
}
