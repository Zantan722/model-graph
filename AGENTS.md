...
同一份 React 編輯器同時是網頁版與 Electron renderer；桌面版可透過 Codex／Codex CLI 讀專案資料夾產圖。
...
- 只有明確啟用才會呼叫真實模型，且只送合成 repo：
  `MODELGRAPH_LIVE_TEST=1 node scripts/test-live-views.cjs`（可加 `MODELGRAPH_TEST_PROVIDER=Codex`）
...
**信任模型**：repo 的檔名與內容一律當成不可信資料。CLI 以純文字轉換器方式使用——無工具權限、空的暫存 cwd（`Codex` 用 `--safe-mode --tools ''`，`codex` 用 read-only sandbox 並關閉全部工具），檔案一律由 App 自己讀。模型回傳的路徑、行號、引用都會對照實際檔案內容重新驗證後才儲存。
