...
const {saveReport,loadReport,saveSources,loadSources}=require('./report-store.cjs');
...
function installAI(getWindow){
 ...
 handle('generateViews',async request=>{
   ...
   try{
     ...
     const result=await generateViews({report,ids:request.viewIds,files,signal:controller.signal,validatePuml:require('./diagram-parser.cjs').importPuml,
     ...
     return {kind:'project',report:result,files:[...new Set(result.themes.flatMap(t=>t.views).filter(v=>request.viewIds.includes(v.id)).flatMap(v=>v.content.items.flatMap(i=>i.evidence.map(e=>e.path))))]};
     ...
     if(request.useFolder){
       ...
       if(projectMode){
         ...
         const report=await analyzeRepository({root:folder.root,manifest,request,signal:controller.signal,
           ask:prompt=>ai.run(executable,ai.generationArgs(request.provider,request.model),prompt,{cwd:temp,signal:controller.signal,timeout:180000}),
           onSources:snapshot=>saveSources(app.getPath('userData'),snapshot,lastCheckpoint),onProgress:emit,onCheckpoint:async report=>{
